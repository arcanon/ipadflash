...
CreateVideoSurface(SDL_Texture * texture)
{
    ...
    fprintf(stderr, "SDL_QueryTexture returned for %X, w %d, h %d\n", format, w, h);
    ...
    if (SDL_QueryTexturePixels(texture, &pixels, &pitch) == 0) {
        fprintf(stderr, "CreateRGBSurfaceFrom %X", pixels);
        surface =
...
ClearVideoSurface()
{
    ...
    black = SDL_MapRGB(SDL_PublicSurface->format, 0, 255, 0);
    if (0 != SDL_FillRect(SDL_PublicSurface, NULL, black))
    {
        fprintf(stderr,"ClearVideoSurface - FillRect Failed\n");
    } else {
        fprintf(stderr,"ClearVideoSurface - FillRect Succeeded\n");
    }
...
SDL_SetVideoMode(int width, int height, int bpp, Uint32 flags)
{
    ...
    if (desktop_format && ((flags & SDL_ANYFORMAT)
        ...
        fprintf(stderr, "using desktop_format\n");
    } else {
        fprintf(stderr, "SDL got bpp %d\n", bpp);
        switch (bpp) {
        case 0:
            if (desktop_format) {
                desired_format = desktop_format;
    ...
    if (!SDL_VideoTexture) {
        fprintf(stderr, "creating with desired format failed\n");
        SDL_VideoTexture =
            SDL_CreateTexture(desktop_format,
                              SDL_TEXTUREACCESS_STREAMING, width, height);
    }
    if (!SDL_VideoTexture) {
        fprintf(stderr, "video format failed\n");
        return NULL;
    }
...
SDL_Surface *
SDL_SetVideoMode2(int width, int height, int bpp, Uint32 flags, void *win, void*view )
{
    SDL_DisplayMode desktop_mode;
    int window_x = SDL_WINDOWPOS_UNDEFINED;
    int window_y = SDL_WINDOWPOS_UNDEFINED;
    ...
    if (SDL_CreateRenderer
        (SDL_VideoWindow, -1,
         SDL_RENDERER_SINGLEBUFFER | SDL_RENDERER_PRESENTDISCARD) < 0) {
        fprintf(stderr,"CreateRenderer failed\n");
        return NULL;
    }
    SDL_GetRendererInfo(&SDL_VideoRendererInfo);
    ...
    if (!SDL_VideoTexture) {
        fprintf(stderr,"creating texture failed\n");
        return NULL;
    }
    ...
    SDL_VideoSurface = CreateVideoSurface(SDL_VideoTexture);
    if (!SDL_VideoSurface) {
        fprintf(stderr,"CreateVideoSurface failed\n");
        return NULL;
    }
    SDL_VideoSurface->flags |= surface_flags;
    ...
    if ((bpp != SDL_VideoSurface->format->BitsPerPixel)
        && !(flags & SDL_ANYFORMAT)) {
        fprintf(stderr,"Creating shadow surface\n");
        SDL_ShadowSurface =
            SDL_CreateRGBSurface(0, width, height, bpp, 0, 0, 0, 0);
        if (!SDL_ShadowSurface) {
            fprintf(stderr,"ShadowSurface failed\n");
            return NULL;
        }
        SDL_ShadowSurface->flags |= surface_flags;
...
{
    int i;
    //fprintf(stderr, "updating surface, x %d y %d, w %d, h %d\n", rects->x, rects->y, rects->w, rects->h);
    ...
    if (screen == SDL_VideoSurface) {
        if (screen->flags & SDL_PREALLOC) {
            //fprintf(stderr, "surface memory is maintained by the renderer\n");
        ...
        if (SDL_VideoRendererInfo.flags & SDL_RENDERER_PRESENTCOPY) {
            fprintf(stderr, "SDL_RENDERER_PRESENTCOPY\n");
            for (i = 0; i < numrects; ++i) {
