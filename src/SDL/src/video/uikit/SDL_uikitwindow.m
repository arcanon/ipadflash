...
static int SetupWindowData(_THIS, SDL_Window *window, UIWindow *uiwindow, SDL_uikitopenglview *uiview, SDL_bool created)
{
    ...
    if (uiwindow)
    {
        ...
        fprintf(stderr, "uiwindow size %.f %.f",uiwindow.frame.size.width,uiwindow.frame.size.height);
    } else
    {
        window->x = 0;
        window->y = 0;
        window->w = (int)uiview.frame.size.width;
        window->h = (int)uiview.frame.size.height;
//        window->w = (int)550;
//        window->h = (int)400;

        fprintf(stderr, "uiview size %.f %.f",uiwindow.frame.size.width,uiwindow.frame.size.height);
    }
    ...
    if ([UIScreen mainScreen] == uiscreen) {
        fprintf(stderr, "ui window is main screec\n");
...
int UIKit_CreateWindow(_THIS, SDL_Window *window, void *win2, void *view) {
    ...
    fprintf(stderr, "creating window %X %X\n",win2, view);
    ...
    if (0)//!win2)
    {
       ...
       if (SDL_UIKit_supports_multiple_displays) {
        ...
        fprintf(stderr,"creating new window \n");
    ...
    {
        fprintf(stderr,"Using window provided \n");
        uiwindow = (UIWindow *)win2;
    }

    fprintf(stderr,"window set \n");
   
    if (SetupWindowData(_this, window, uiwindow,(SDL_uikitopenglview *)view, SDL_TRUE) < 0) {
        fprintf(stderr,"SetupWindowData failed \n");
        [uiwindow release];
        return -1;
    }    
...
void UIKit_ShowWindow(_THIS, SDL_Window * window) {
    SDL_WindowData *data = (SDL_WindowData *)window->driverdata;
    fprintf(stderr,"Trying to make the window visible\n");
    if (data) {
        fprintf(stderr,"Making the window visible\n");
        [window makeKeyAndVisible];        
    }
    fprintf(stderr,"Done window visible\n");
}
