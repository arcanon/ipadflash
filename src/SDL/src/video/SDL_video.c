...
SDL_VideoInit(const char *driver_name, Uint32 flags)
{
    ...
    if (video == NULL) {
        if (driver_name) {
            ...
            SDL_SetError("No available video device, available videos %d", video_count);
        }
        return -1;
    }
    _this = video;
...
SDL_Window *
SDL_CreateWindow(const char *title, int x, int y, int w, int h, Uint32 flags, void * win, void* view)
{
    ...
    if (_this->CreateWindow && _this->CreateWindow(_this, window, win, view) < 0) {
...
SDL_RecreateWindow(SDL_Window * window, Uint32 flags)
{
    ...
    if (_this->CreateWindow && !(flags & SDL_WINDOW_FOREIGN)) {
        if (_this->CreateWindow(_this, window,0,0) < 0) {
            if (flags & SDL_WINDOW_OPENGL) {
                SDL_GL_UnloadLibrary();
            }
            return -1;
...
SDL_ShowWindow(SDL_Window * window)
{
    ...
    fprintf(stderr,"SDL_ShowWindow %x Shown_flag %x", window->flags, SDL_WINDOW_SHOWN);

    if (window->flags & SDL_WINDOW_SHOWN) {
        return;
    }
    
    fprintf(stderr,"SDL_ShowWindow - calling host function");
...
SDL_CreateTexture(Uint32 format, int access, int w, int h)
{
    ...
    if (!renderer) {
        fprintf(stderr,"GetCurrentRenderer dead\n");
        return 0;
    }
    if (!renderer->CreateTexture) {
        fprintf(stderr,"Unsupported textures\n");
        SDL_Unsupported();
        return 0;
    }
    if (w <= 0 || h <= 0) {
        fprintf(stderr,"Texture dimensions can't be 0\n");
        return 0;
    }
    ...
    if (renderer->CreateTexture(renderer, texture) < 0) {
        fprintf(stderr,"Create call driver failed\n");
...
//    fprintf(stderr, "srcrect x %d y %d w %d h %d dstrect x %d y %d w %d h %d\n" , 
...
//            dstrect->x,dstrect->y,dstrect->w,dstrect->h);
    ...
    if (texture->renderer != renderer) {
        fprintf(stderr, "Texture was not created with this renderer\n");
        return -1;
    }
    if (!renderer->RenderCopy) {
        fprintf(stderr, "Error no renderer copy\n");
        SDL_Unsupported();
        return -1;
    }
    ...
    if (srcrect) {
        if (!SDL_IntersectRect(srcrect, &real_srcrect, &real_srcrect)) {
            fprintf(stderr, "Does not intersect leaving, texw %d texh %d\n", texture->w, texture->h);
            return 0;
        }
    }
    ...
    if (dstrect) {
        if (!SDL_IntersectRect(dstrect, &real_dstrect, &real_dstrect)) {
            fprintf(stderr, "Does not intersect leaving, windoww %d windowh %d\n", window->w, window->h);
            return 0;
        }
