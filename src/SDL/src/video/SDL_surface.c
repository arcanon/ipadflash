                     ...
                     Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
    ...
    if (!surface->format) {
        fprintf(stderr,"Allocating surface failed\n");
    ...
    if (surface->format->BitsPerPixel <= 8) {
        ...
        if (!palette) {
            fprintf(stderr,"Allocating palette failed\n");
            SDL_FreeSurface(surface);
            return NULL;
        }
    ...
    if (!surface->map) {
        fprintf(stderr,"Allocating Blit failed\n");
        SDL_FreeSurface(surface);
        return NULL;
    }
              ...
              SDL_Surface * dst, SDL_Rect * dstrect)
{
    ...
    if ((src->map->dst != dst) ||
        ...
        /* just here for debugging */
         fprintf 
             (stderr, "src = 0x%08X src->flags = %08X src->map->info.flags = %08x\ndst = 0x%08X dst->flags = %08X dst->map->info.flags = %08X\nsrc->map->blit = 0x%08x\n", 
